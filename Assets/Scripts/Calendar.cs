...
public class Calendar : MonoBehaviour
{
    ...
    public class Day
    {
        ...
        public Day(int dayNum, Color dayColor, GameObject obj, DateTime currentDate)
        {
            this.dayNum = dayNum;
            this.obj = obj;
            this.currentDate = new DateTime(currentDate.Year, currentDate.Month, dayNum + 1);
        ...
        if(days.Count == 0)
        {
            for (int w = 0; w < 6; w++)
            {
                for (int i = 0; i < 7; i++)
                {
                    ...
                    {
                        newDay = new Day(currDay - startDay, Color.white,weeks[w].GetChild(i).gameObject, temp);
                    }
                    days.Add(newDay);
                }
            }
